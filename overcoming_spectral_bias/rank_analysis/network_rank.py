...
class Q_mlp(nn.Sequential):
    def __init__(self, n_layers=4, activation=nn.ReLU):
        self.n_layers = n_layers
        layers = []
        for _ in range(n_layers - 1):
            layers += [nn.Linear(400, 400), activation()]
        super().__init__(
...
def collect_rank(network: str, mode: str, H, gamma, seed):
    from ml_logger import logger
    from overcoming_spectral_bias.rank_analysis.rand_mdp import ToyMDP
    from overcoming_spectral_bias.rank_analysis.network_rank import Q_mlp, Q_rff, supervised, perform_deep_vi, \
        vi_by_horizon
    ...
    gt_q_values = vi_by_horizon(rewards, dyn_mats, H=H, gamma=gamma)

    if network == 'mlp':
        Q = Q_mlp()
    elif network == 'mlp-deep':
        Q = Q_mlp(n_layers=8)
    elif network == 'mlp-deeper':
        Q = Q_mlp(n_layers=12)
    elif network == 'mlp-tanh':
        Q = Q_mlp(activation=nn.Tanh)
    elif network == 'rff':
        Q = Q_rff(B_scale=10)

    if mode == 'supervised':
        q_values, losses = supervised(Q, states, gt_q_values.T, n_epochs=400, batch_size=32)
    elif mode == 'fqi':
    ...
    logger.log(rank=rank.item())
    logger.flush()

def launch(start_seed, n, network: str, mode: str, H, gamma):
    from overcoming_spectral_bias.rank_analysis.network_rank import collect_rank
    from ml_logger import logger

    logger.configure(f"conjugate-kernel/conjugate-kernel/rank-experiment/04/")

    with logger.Prefix(f"{network}/{mode}/{H}/{gamma}"):
        logger.job_running()
        for seed in range(start_seed, start_seed + n):
            collect_rank(network, mode, H, gamma, seed=seed)
...
if __name__ == '__main__':
    ...
    for network in ['mlp', 'rff']:
        for start_seed in [0, 4, 8, 12]:
        # start_seed = 0
            H = 200
            mode = 'supervised'
            for gamma in [0.1, 0.6, 0.9, 0.99]:
                jaynes.config(runner=dict(name=f"{network}/{mode}/{H}/{gamma}-{logger.now('%f')}", n_gpu=None))
                jaynes.run(launch, start_seed, 4, network, mode, H, gamma)
...
if __name__ == '__main__':
    ...
    jaynes.config(runner=dict(n_gpu=None))

    for network in ['mlp', 'rff', 'mlp-deeper', 'mlp-tanh']:
        for start_seed in range(0, 100, 10):
            H = 200
            for gamma in [0.1, 0.6, 0.9, 0.99]:
                jaynes.run(launch, start_seed, 10, network, 'supervised', H, gamma)
...
if __name__ == "__main__":
    doc @ """
    We collect the rank of the networks via map-reduce on the cluster.
