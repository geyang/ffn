...
if __name__ == '__main__':
    ...
    colors = ['#23aaff', '#9799f9', '#d784db', '#ff7777'][::-1]

    for row, band_limit, c in zip(cross_section, band_limits, colors):
        plt.figure('cross section', figsize=(5, 3))
    ...
    for row, band_limit, c in list(zip(cross_section, band_limits, colors))[::-1]:
        plt.figure('spectrum', figsize=(5, 3))
        fft = np.fft.fft(row)
    ...
    plt.savefig(f"{Path(__file__).stem}/fourier_features_ntk_cross.pdf")
    ...
    plt.savefig(f"{Path(__file__).stem}/fourier_features_ntk_spectrum.pdf")
