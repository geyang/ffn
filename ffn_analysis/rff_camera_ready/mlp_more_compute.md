...
for (e, (env, scale, comp_rg)) in enumerate(tqdm(zip(envs, scales, compute_range), desc="env-scales")):
    ...
    for (j, update_freq) in enumerate(comp_rg):
        plot_line(path=f"compute/mlp/{env}/update_opt_steps-{update_freq}/**/metrics.pkl",
                  label=f'MLP (freq={update_freq})', color='black',
                  x_key='frames', y_key="eval/episode_reward/mean", marker=markers[j])
...
|:--------------:|:-----------------:|
| <img style="align-self:center; zoom:0.3;" src="mlp_more_compute/Walker-run.png" image="None" styles="{'margin': '0.5em'}" width="None" height="None" dpi="300"/> | <img style="align-self:center; zoom:0.3;" src="mlp_more_compute/Quadruped-run.png" image="None" styles="{'margin': '0.5em'}" width="None" height="None" dpi="300"/> |
