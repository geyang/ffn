...
for (e, env) in enumerate(tqdm(envs, desc="env")):
    ...
    plot_line(path=f"sac_dennis_rff/dmc/2_layer/lff/{env}/alpha_tune/scale-0.001/**/metrics.pkl",
              color=colors[0], label=f'FFN',
              x_key='frames', y_key="eval/episode_reward/mean")

    for (i, scale) in enumerate(scales):
        plot_line(path=f"sac_dennis_rff/dmc/actor_scale_ablate/lff/{env}/alpha_tune/actor_scale-{scale}/**/metrics.pkl", color=colors[i+1], label=f'FFN (Actor B={scale})',
                    x_key='frames', y_key="eval/episode_reward/mean")
...
| **Walker-run** |
|:--------------:|
| <img style="align-self:center; zoom:0.3;" src="actor_scale_ablate/Walker-run.png" image="None" styles="{'margin': '0.5em'}" width="None" height="None" dpi="300"/> |
