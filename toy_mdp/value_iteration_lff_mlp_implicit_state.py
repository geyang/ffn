...
class Q_implicit(nn.Module):
    def __init__(self, state_dim, action_dim, latent_dim=400, ff=None, B_scale=1, latent_scale=1):
        ...
        if ff == 'RFF':
            self.ff = RFF(state_dim, latent_dim//2, scale=B_scale)
            ...
            self.ff = LFF(state_dim, latent_dim // 2, scale=B_scale)
            self.Q = nn.Sequential(
                LFF(latent_dim+action_dim, latent_dim // 2, scale=latent_scale),
    ...
    def forward(self, states):
        ...
        for act in range(self.action_dim):
            ...
            if self.ff:
                ff_states = self.ff(states)
                values = self.Q(torch.cat([ff_states, actions], dim=1))
            else:
